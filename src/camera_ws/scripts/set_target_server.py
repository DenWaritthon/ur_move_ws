...
from target_interfaces.srv import DetectionTarget  # Ensure this matches your package
...
class RedBoxDetectorService(Node):
    ...
    def get_latest_image(self):
        ...
        self.latest_image = None  # Reset before waiting
        self.get_logger().info("Waiting for an image...")

        timeout_sec = 5.0
        start_time = self.get_clock().now().to_msg().sec
        while (self.latest_image is None) and ((self.get_clock().now().to_msg().sec - start_time) < timeout_sec):
